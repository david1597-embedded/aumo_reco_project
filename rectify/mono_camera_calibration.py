...
def calibrate_camera():
    ...
    # 이미지 디렉토리 경로
    image_dir = "./rectify/captured_image_right"
    calibration_dir = "./rectify/calibration_data"
    ...
    # 이미지 파일 찾기
    image_files = []
    for i in range(1, 16):  # IMG_right_1.jpg부터 IMG_right_15.jpg까지
        filepath = os.path.join(image_dir, f"IMG_right_{i}.jpg")
    ...
    # 캘리브레이션 결과 저장
    calibration_file = os.path.join(calibration_dir, "right_camera_parameter.npz")
    ...
    print(f"   성공한 이미지 (디버그): {calibration_dir}/debug_IMG_right_*.jpg")
    print(f"   실패한 이미지 (디버그): {calibration_dir}/failed_IMG_right_*.jpg")
